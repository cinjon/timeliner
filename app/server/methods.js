Meteor.methods({
    create_clip: function(data) {
        ...
        var count = Clips.find({episode_id:episode_id, $where: "this.end >= " + start + " && " + end + " >= this.start"}).count(); //clip_collision
        ...
        if (count == 0) {
            ...
            for (var i = 0; i < links.length; i++) {
                ...
                link_obj = Links.findOne({url:link.url});
                if (!link_obj) {
                    link_id = Links.insert({url:link.url, text:link.text, created_at:ts});
                } else {
            ...
            Episodes.update({_id:episode_id}, {$pushAll:{links:link_ids}});
        } else {
